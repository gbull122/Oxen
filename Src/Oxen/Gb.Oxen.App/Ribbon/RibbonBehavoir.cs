...
public class RibbonBehavoir : RegionBehavior
{
    ...
    protected void MergeTabs(Fluent.Ribbon moduleRibbon)
    {
        ...
        foreach (var tab in tabs)
        {
            ...
            if (tab.Header == "Main" && MainRibbon.Tabs.Any(a => a.Header == "Main"))
            {
    ...
    protected void MergeMainTab(RibbonTabItem tabItem)
    {
        foreach (RibbonGroupBox rgb in tabItem.Groups)
        {
            DisconnectFromParent(tabItem, rgb);
            MainRibbon.Tabs[0].Groups.Add(rgb);
        }
    }
